      ...
      <b-col>
        ...
        <b-row>
          <b-container fluid>
            <p>Profile:</p>
            <b-row class="my-1"
              v-for="(type, label) in form_inputs"
              :key="label">
              <b-col sm="2">
                <label :for="`type-${type}`">{{ label }}:</label>
              </b-col>
              <b-col sm="6">
                <b-form-input :id="`type-${type}`"
                  :type="type"
                  v-model="form_outputs[label]"
                  :placeholder="current_settings[label]"></b-form-input>
              </b-col>
            </b-row>
          </b-container>
        </b-row>
        ...
        <b-row>
          <b-container fluid>
            <p>Reset Passwords:</p>
            <b-row class="my-1"
              v-for="(type, label) in newpasswords_inputs"
              :key="label">
              <b-col sm="2">
                <label :for="`type-${type}`">{{ label }}:</label>
              </b-col>
              <b-col sm="6">
                <b-form-input :id="`type-${type}`"
                  :type="type"
                  v-model="newpasswords_outputs[label]"></b-form-input>
              </b-col>
            </b-row>
          </b-container>
        </b-row>
        ...
        <b-row class="m-2">
          <b-col-5 class="m-2">
            <button type="button"
              ...
              v-on:click="output">Save</button>
          </b-col-5>
          <b-col-5 class="m-2">
            <button type="button"
              class="btn btn-primary btn-lg btn-block">Cancel</button>
          </b-col-5>
        </b-row>
      </b-col>
    </b-row>
...
export default {
  ...
  created() {
    xeConnectorApiService
      .getSettings({ session_id: Cookies.get('name') })
      .then((response) => {
  ...
  methods: {
    output(event) {
      ...
      if (event) {
        xeConnectorApiService
          .userUpdateSettings(this.form_outputs)
          .then((response) => {
            // eslint-disable-next-line
        ...
        if (
          this.newpasswords_outputs.New === this.newpasswords_outputs.Confirm &&
          this.newpasswords_outputs.New !== this.newpasswords_outputs.Old
        ) {
          xeConnectorApiService
            .userChangePassword(changePssswordRequest)
            .then((response) => {
              // eslint-disable-next-line
              console.log(response);
            })
